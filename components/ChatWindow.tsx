...
type ChatWindowProps = {
  /** A query forwarded from the Hero Smart Console "Calculate" button, sent once on mount. */
  pendingQuery: string;
  onQueryConsumed: () => void;
  onClose: () => void;
  /** Pushes the window up above the cookie banner while it's shown. */
  bottomOffset: number;
};
...
export default function ChatWindow({
  ...
  // Forward the Hero Smart Console query, if any, once on mount.
  useEffect(() => {
    if (pendingQuery) {
      onQueryConsumed();
      setTimeout(() => sendMessage(pendingQuery), 600);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
